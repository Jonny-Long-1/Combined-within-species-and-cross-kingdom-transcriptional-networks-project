...
Input_1_wide <- read.csv("For_writeup/3_layer_network_work/Nematode_effectors_cyst.csv", stringsAsFactors = FALSE)
#Take a csv file containing all genes of the "minor" type - Input 2 (These will only have edges with the genes of major type)
Input_2_wide <- read.csv("For_writeup/3_layer_network_work/Nematode_TFs_cyst.csv", stringsAsFactors = FALSE)
...
output_att_name <- "output_attribute_file_3_layer_et_0.975" 

#ENTER DESIRED NAME FOR OUTPUT COEFFICIENT FILE (do not include file ext)
output_coeff_name <- "output_coefficient_file_3_layer_et_0.975" 
...
exp_category_names <- c("Cyst","J2","Hours_10", "Hours_48", "Days_12_Female", "Days_12_Male", "Days_24_Female")
...
output_coeff_total_3_layer_et <- rbind(output_coeff1s, output_coeff12s)
...
output_att_total_3_layer_et <- rbind(att_data1, att_data2) %>% column_to_rownames("gene")


#Save outputs

#if(attribute_file_yes == TRUE){
#  output_att_name <- paste0("For_writeup/",output_att_name, ".txt")
#  write.table(output_att_total, output_att_name, row.names = TRUE, col.names = FALSE, sep = "\t",quote = FALSE)
#}

#output_coeff_name <- paste0("For_writeup/",output_coeff_name,".txt")
