...
att <- read.table("For_writeup/output_attribute_file_659_plant_0.975.txt", sep = "\t", header = FALSE, quote = "", dec=".")

#ENTER COEFFICIENTS INPUT
coeff <- read.table("For_writeup/output_coefficient_file_659_plant_0.975.txt", sep = "\t", header = FALSE, quote = "", dec=".")


#ENTER DESIRED OUTPUT NAME (exclude file ext)
final_gexf <- "For_writeup/695_effector_plant_network"

#ENTER NODE ATTRIBUTE NAMES
node_att_names <- c("Cluster", "Kingdom","Kingdom:cluster","TF","3d_calc","Response to biotic stimulus","Response to stress","RBS and stress","Anatomical_development","multicellular organ development","post embryonic development","flower_development","any_development","Annotations of interest","AUC","Proportion negative","neg class","Defence/Immunity gene","Colour_by")
