...
Input_1_wide <- read.csv("For_writeup/3_layer_network_work/Nematode_effectors_non_cyst.csv", stringsAsFactors = FALSE)
#Take a csv file containing all genes of the "minor" type - Input 2 (These will only have edges with the genes of major type)
Input_2_wide <- read.csv("For_writeup/3_layer_network_work/Arabidopsis_data.csv", stringsAsFactors = FALSE)
...
output_att_name <- "sugr_prox_output_attribute_file_3_layer_final_0.975" 

#ENTER DESIRED NAME FOR OUTPUT COEFFICIENT FILE (do not include file ext)
output_coeff_name <- "sugr_prox_output_coefficient_file_3_layer_final_0.975" 
...
coeff12s <- matrix(nrow = no_of_genes1, ncol = no_of_genes2, dimnames = list(gene_name_list1, gene_name_list2))
print(no_of_genes2)

for(i in 1:no_of_genes1){
  print(i)
  P <- aov1[,i] 
  for(j in 1:no_of_genes2){
    O <- aov2[,j]
    pearsons <- cor(P,O)
    if(is.na(pearsons) == FALSE && pearsons<0){
...
nodesc <- data.frame(output_coeff12s$Var1) %>%  rename("gene" = output_coeff12s.Var1)

#The column containing "minor" genes involved in edges
nodesd <- data.frame(output_coeff12s$Var2) %>%  rename("gene" = output_coeff12s.Var2)
...
output_coeff_final_3_layer <- rbind(output_coeff_total_3_layer_et, output_coeff12s)
...
#att_data1 <- left_join(new_gene_name_list1, att_data1) <- Now we want to keep all the effectors in the network
att_data2 <- left_join(new_gene_name_list2, att_data2) %>% column_to_rownames("gene")

#Combine all remaining nodes and attributes into a single table containing them all
output_att_final_3_layer <- rbind(output_att_total_3_layer_et, att_data2)


#Save outputs

if(attribute_file_yes == TRUE){
  output_att_name <- paste0("For_writeup/3_layer_network_work/",output_att_name, ".txt")
  write.table(output_att_final_3_layer, output_att_name, row.names = TRUE, col.names = FALSE, sep = "\t",quote = FALSE)
}

output_coeff_name <- paste0("For_writeup/3_layer_network_work/",output_coeff_name,".txt")
write.table(output_coeff_final_3_layer, output_coeff_name, row.names = FALSE, col.names = FALSE, sep = "\t",quote = FALSE)
